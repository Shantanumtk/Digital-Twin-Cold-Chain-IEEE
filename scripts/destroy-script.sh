#!/bin/bash
# =============================================================================
# Cold Chain Digital Twin — Idempotent Destroy Script (with Phase 5 cleanup)
# =============================================================================
# Tears down all resources in reverse order.
# Handles ECR image cleanup, MCP Agent cleanup, and CloudWatch log group
# pre-deletion to prevent Terraform destroy failures.
...
confirm_destroy() {
  ...
  echo -e "    • MCP Agent container on MQTT EC2"
...
# =============================================================================
# Step 2: Stop MCP Agent on MQTT EC2
# =============================================================================
destroy_mcp_agent() {
  log_step "Stop MCP Agent on MQTT EC2"

  # Get MQTT EC2 public IP
  MQTT_IP=$(aws ec2 describe-instances \
    --filters "Name=tag:Name,Values=${PROJECT}-mqtt-broker" "Name=instance-state-name,Values=running" \
    --query "Reservations[0].Instances[0].PublicIpAddress" --output text 2>/dev/null || true)

  if [ -z "$MQTT_IP" ] || [ "$MQTT_IP" = "None" ]; then
    log_skip "MQTT EC2 not found or not running"
    track_step "MCP Agent Cleanup" "skip" "EC2 not found"
    return 0
  fi

  log_info "MQTT EC2: $MQTT_IP — stopping MCP Agent..."

  ssh -i "$SSH_KEY" -o StrictHostKeyChecking=no -o ConnectTimeout=10 ${SSH_USER}@${MQTT_IP} << 'REMOTE_EOF' 2>/dev/null || true
docker stop mcp-agent 2>/dev/null && echo "Stopped mcp-agent" || echo "mcp-agent not running"
docker rm mcp-agent 2>/dev/null || true
docker rmi mcp-agent:latest 2>/dev/null || true
rm -rf ~/mcp-agent/.env 2>/dev/null || true
REMOTE_EOF

  log_done "MCP Agent stopped and cleaned up"
  track_step "MCP Agent Cleanup" "pass" "Container removed"
}

# =============================================================================
# Step 3: Delete K8s Workloads & Namespace
# =============================================================================
destroy_kubernetes() {
  ...
  # Delete services (including Phase 5 NodePort services)
  log_info "Deleting services..."
  for svc in state-engine dashboard kafka kafka-headless kafka-external redis redis-external redis-headless mqtt-kafka-bridge kafka-consumer; do
  ...
  PVC_COUNT=$(kubectl get pvc -n "$NAMESPACE" --no-headers 2>/dev/null | wc -l | tr -d ' ' || echo "0")
...
# =============================================================================
# Step 4: Wait for LoadBalancers to Deprovision
# =============================================================================
wait_for_lb_cleanup() {
...
# =============================================================================
# Step 5: Delete ALL ECR Images & Repositories
# =============================================================================
delete_ecr_images() {
...
main() {
  ...
  echo "  ║                                                       ║"
  echo "  ║   Tears down: MCP Agent │ EKS │ EC2 │ VPC │ ECR      ║"
  echo "  ║   Preserves:  S3 state bucket                        ║"
  echo "  ╚═══════════════════════════════════════════════════════╝"
  echo -e "${NC}"

  confirm_destroy          # Step 1
  destroy_mcp_agent        # Step 2  ← NEW
  destroy_kubernetes       # Step 3
  wait_for_lb_cleanup      # Step 4
  delete_ecr_images        # Step 5
