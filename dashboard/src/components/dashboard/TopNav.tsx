...
export default function TopNav({ page, subNav, onSubNavChange, profileName = "demo" }: TopNavProps) {
  ...
  return (
    <div style={{
      ...
      <span style={{ fontSize: 14, fontWeight: 700, color: theme.text, marginRight: 16 }}>{pageLabel}</span>
      ...
      <div style={{ marginLeft: "auto", display: "flex", alignItems: "center", gap: 10 }}>
        ...
        <span style={{ fontSize: 10, color: theme.accent, fontWeight: 500 }}>Online</span>
        <div style={{ width: 1, height: 18, background: theme.border, margin: "0 4px" }} />
        <span style={{ fontSize: 11, color: theme.dim }}>Profile: {profileName}</span>
      </div>
    </div>
  );
}
